...
// request is higher order function
    ...
    for(let i = 0 ; i < bothBowlingTables.length ; i++)
    {
        ...
        for(let j = 0 ; j < allTableRows.length ; j++)
        {
            ...
            if(i == 0 && j == 0)
            {
                ...
                if(currentWicket > highestWicketsTaken || (currentWicket == highestWicketsTaken && currentEconomy < economyOfHighestWicketTaker))
                {
                    // update if current bowler have high wickets and if they have same wicket and lower economy then also it gets updated 
                    highestWicketTakenName = myDocument(allTds[0]).find("a").text() ;
                    highestWicketsTaken = currentWicket ;
